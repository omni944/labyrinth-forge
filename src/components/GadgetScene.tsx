...
export function GadgetScene({ data, resetSignal }: { data: GadgetGeometryData; resetSignal: number }) {
  ...
  const isFlatOrnament = data.layout === 'assembled'
  const ornamentCenterZ = isFlatOrnament ? (data.depth - data.width) / 2 : 0
  const cameraPosition: [number, number, number] = isFlatOrnament
    ? [0, size * 2.05, ornamentCenterZ]
    : [size * 1.15, size * 1.2, size * 1.35]
  return (
    <Canvas dpr={[1, 2]} shadows gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping }}>
      ...
      <PerspectiveCamera key={`gadget-camera-${resetSignal}`} makeDefault position={cameraPosition} rotation={isFlatOrnament ? [-Math.PI / 2, 0, 0] : undefined} fov={isFlatOrnament ? 38 : 42} near={0.1} far={size * 12} />
      <OrbitControls key={`gadget-controls-${resetSignal}`} makeDefault target={[0, data.height * 0.42, ornamentCenterZ]} minDistance={size * 0.3} maxDistance={size * 4} maxPolarAngle={Math.PI / 2.03} enableDamping dampingFactor={0.07} />
